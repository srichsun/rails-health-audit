...
├── static-scan-report.md      # 靜態掃描：摘要表 + 排好序的 Action plan
...
`static-scan-report.md` 是你拿來讀、拿來行動的那份；`raw/` 是細節所在。
...
或者不用跑，直接看已提交的輸出快照
[`examples/legacy_blog/sample-static-scan-report.md`](examples/legacy_blog/sample-static-scan-report.md)。
