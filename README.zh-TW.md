...
`scripts/pass2.sh` 把第一組自動化了：它啟動 app、用**暫時的** bundle 跑資料正確性與索引
...
**第二輪（runtime）。** 對一個資料庫已設定、已 migrate 的專案：
...
並寫出 `<project>/tmp/health-audit/PASS2.md`。
...
- `pass2.sh` 把資料正確性與索引檢查自動化了，但 N+1 與覆蓋率仍需 app 被**實際執行到**
  （請求 / 測試），所以那兩項維持手動。
